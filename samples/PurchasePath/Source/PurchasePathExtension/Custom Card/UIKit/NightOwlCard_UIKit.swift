...
class NightOwlCard_UIKit: Card {
    
    var isRedeemed: Bool = false
    var redeemedImage: UIImage?
    
    override static func createView() -> UIView {
        return NightOwlCardView_UIKit.nightOwlCardView()
    }
    
    override func prepareView(_ cardView: UIView) {
        guard let nightOwlCardView = cardView as? NightOwlCardView_UIKit else {
            return
        }
    ...
    func updateView(view: NightOwlCardView_UIKit) {
    ...
    @objc func redeemAction(sender: Any) {
        guard let cardView = self.view as? NightOwlCardView_UIKit else {
            return
        }

        // Simulated async task
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) { [self] in
            redeemedImage = UIImage(named: "moneyowl")
            isRedeemed = true
